...
function processDetectionState() {
  ...
  // We need at least 6 samples to calculate delta over 500ms
  if (brightnessHistory.length < 6) return;
  
  const current = brightnessHistory[brightnessHistory.length - 1];
  const prev500ms = brightnessHistory[brightnessHistory.length - 6];
  const delta = current.average - prev500ms.average;
  
  // Define dynamic thresholds based on ambient light average
  const dropRequired = -ambientBrightness.average * 0.16; // Drop of 16% of ambient light (more sensitive)
  const riseRequired = ambientBrightness.average * 0.12;  // Rise of 12% of ambient light (more sensitive)
  
  // Calculate spatial contrast (Periphery - Center)
  const contrast = current.periphery - current.center;
  
  // Under low light, we bypass contrast check because camera noise dominates
  const contrastCondition = isLowLightMode ? true : (contrast > contrastThreshold);
  
  switch (detectionState) {
    case "IDLE":
      // Look for sudden drop AND center becoming darker than periphery (contrast) if in normal light
      if (delta < dropRequired && contrastCondition && (now - lastSajdahTime > SUJUD_COOLDOWN)) {
        detectionState = "DOWN_PENDING";
        stateStartTime = now;
        console.log(`Delta drop detected (LowLight:${isLowLightMode}): Δ:${delta.toFixed(1)}%, Contrast:${contrast.toFixed(1)}% (req:${contrastThreshold.toFixed(1)}%), moving to DOWN_PENDING`);
      }
      break;
      
    case "DOWN_PENDING":
      // Verify the drop is deep enough to confirm Sujud
      if (current.average < ambientBrightness.average * 0.75) {
        if (now - stateStartTime >= CONFIRM_DOWN_TIME) {
          detectionState = "IN_SUDJUD";
          onSujudDown();
          console.log("Confirmed Sujud (DOWN)");
        }
      } else if (current.average > ambientBrightness.average * 0.85) {
        // Only revert to IDLE if light actually recovers back to near-ambient (flicker)
        detectionState = "IDLE";
        console.log("Flicker detected (light recovered), reverting to IDLE");
      }
      break;
      
    case "IN_SUDJUD":
      // Look for sudden rise AND periphery clearing first (positive contrast) if in normal light
      if (delta > riseRequired && contrastCondition) {
        detectionState = "UP_PENDING";
        stateStartTime = now;
        console.log(`Delta rise detected (LowLight:${isLowLightMode}): Δ:${delta.toFixed(1)}%, Contrast:${contrast.toFixed(1)}%, moving to UP_PENDING`);
      }
      break;
      
    case "UP_PENDING":
      // Verify the rise persists (standing up confirmation)
      if (current.average > ambientBrightness.average * 0.65) {
        if (now - stateStartTime >= CONFIRM_UP_TIME) {
          detectionState = "IDLE";
          onSujudUp();
          console.log("Confirmed Rise (UP)");
        }
      } else if (current.average < ambientBrightness.average * 0.40) {
        // If it got very dark again, revert back to IN_SUDJUD (still in Sujud)
        detectionState = "IN_SUDJUD";
        console.log("Flicker detected (got dark again), reverting to IN_SUDJUD");
      }
      break;
  }
}
